...
async def create_scaffold(update: Update, context: ContextTypes.DEFAULT_TYPE, assistant):
    ...
    Knowledge: To fulfill your task you will use your general knowledge. There is not document attached. Do not complain.
    Enumeration: Enumerate parent chapters incrementally starting from 1. For the subchapters use as a prefix the number of the parent chapter followed by a dot and add a suffix enumerating from 1.
    Format of your response: We want to later iterate through your response, therefore return your response as an enumerated list where each chapter or sub-chapter is a record followed by a carriage-return character. 
